...
    essay : "${essay}"`;
  ...
  async brainstormTopic(topic: string) {
    const prompt = `Brainstorm key ideas, arguments, and supporting points for the following IELTS Writing Task 2 topic: "${topic}".Do not include extra text`;
    return await this.callAIApi({ prompt: prompt });

  }

  async findSynonymAndAnonyms(word: string) {
    const prompt = `Provide at least 1 and at most 5 synonyms and antonyms for the word ${word}. Ensure the synonyms are contextually appropriate and reflect different shades of meaning. If the word has multiple meanings, focus on the most common usage. Return the output strictly in valid JSON format with the structure below.
      {
